...
| Replay or duplicated run/effect | Atomic semantic-run store plus stable semantic-effect idempotency in the action cell | Defaults are per runner; protection across runners requires injected shared stores, which remain non-durable |
...
| Activation expires during run | Expiry is part of the effective deadline and activation is revalidated at every dispatch boundary | Synchronous callback cannot be preempted |
...
| Mutable/untrusted JSON | Strict detached plain-JSON snapshots before hashing/reuse | Application-owned callbacks remain trusted code |
| Target/argument confusion | Segment-aware prefixes, host-owned validators, canonical HTTPS destination origin, same-origin binding for URL-like targets, and rejection of destinations on non-network scopes | Opaque target/arguments must remain data identifiers; executor-derived endpoints and every redirect hop must use the same trusted resolution and egress checks |
...
- exact-run replay and duplicate semantic-effect replay across changed roots or
...
- nested spawn and approval-required configuration;
